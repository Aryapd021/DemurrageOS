...
export default function DocumentViewerPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
              ...
              <span className="text-slate-400 dark:text-slate-500 font-normal text-sm">Parsed Trade Documents</span>
            </h1>
          </div>
        </div>
        ...
        {activeDoc && activeDoc.status !== "REVIEWED" && (
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-lg shadow-sm transition-colors cursor-pointer"
          >
      ...
      {acceptedNotice && (
        <div className="bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900/50 text-emerald-900 dark:text-emerald-300 rounded-lg p-3 text-xs font-semibold flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            {documents.map((doc) => (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-semibold transition-all cursor-pointer ${
                  (activeDoc?.id === doc.id)
                    ? "bg-white dark:bg-[#0c1424] text-slate-900 dark:text-white shadow-sm"
        ...
        <div className="lg:col-span-5 bg-white dark:bg-[#0c1424] rounded-xl border border-slate-200 dark:border-white/5 p-5 shadow-sm dark:shadow-[0_4px_24px_rgba(0,0,0,0.3)] space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-white/5 pb-3">
            <div className="flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-purple-600 dark:text-purple-400" />
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">Extracted Entities &amp; Confidence</h3>
            </div>
            {activeDoc?.status === "REVIEWED" ? (
              <span className="text-xs bg-emerald-100 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300 font-bold px-2 py-0.5 rounded flex items-center gap-1 border border-emerald-200 dark:border-emerald-900">
                <Check className="w-3 h-3" />
                Verified
              </span>
            ) : (
              <span className="text-xs bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 font-bold px-2 py-0.5 rounded flex items-center gap-1 border border-amber-200 dark:border-amber-900">
                  ...
                  <div
                    ...
                    className={`p-3 rounded-lg border text-xs transition-colors ${
                      field.flaggedForReview
                        ? "bg-amber-50/70 dark:bg-amber-950/20 border-amber-300 dark:border-amber-900/40"
                        : "bg-slate-50/60 dark:bg-white/5 border-slate-200 dark:border-white/10"
                    }`}
                  >
                    <div className="flex items-center justify-between">
                    ...
                    {isEditing ? (
                      <div className="mt-2 flex items-center gap-2">
                        <input
                          ...
                          className="flex-1 bg-white dark:bg-slate-900 border border-blue-400 rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                        <button
                      ...
                      <div className="font-semibold text-slate-800 dark:text-slate-200 mt-1 font-mono break-all">
                        {String(field.value)}
                      </div>
                    )}
          ...
          <div className="bg-slate-50 dark:bg-white/5 rounded-lg p-3 border border-slate-200 dark:border-white/10 text-[11px] text-slate-600 dark:text-slate-400 space-y-1">
